...
async function startneoxy() {
    ...
    const neoxy = neoxyConnect({
        ...
        browser: ['NeoBotz - MD','Safari','1.0.0'],
        auth: state,
        version
    })
...
*Yhaha Beban Group Telah Berkurang Jangan Lupa Kalo Mau Balik Bawa Gorengan:v*
` 
...
Left = await getBuffer(`https://leyscoders-api.herokuapp.com/api/leave?name=${nama}&group=${encodeURIComponent(metadata.subject)}&profile=${ppuser}&apikey=IkyOgiwara`)
                ...
                if (anu.action == 'add') {
                    neoxy.sendButWel(anu.id, cangton, `© Created By Juvian Official `, Welcome, vtbtn)
                } else if (anu.action == 'remove') {
                   neoxy.sendButWel(anu.id, cangtion, `© Created By Juvian Official `, Left, bkgc)
                }
            }
        } catch (err) {
            console.log(err)
